					...
					if( ! empty($price_calculator_array) && is_array($price_calculator_array) ) {
						...
						if( isset($orderitem_measurement_data) ) {
							if( ! empty ($orderitem_measurement_data['length']['value']) ) {
								$package['contents'][$key]['length'] = wc_get_dimension( $orderitem_measurement_data['length']['value'], $wc_dimension_unit, $orderitem_measurement_data['length']['unit'] ) ;
							}
							if( ! empty($orderitem_measurement_data['width']['value']) ) {
								$package['contents'][$key]['width'] = wc_get_dimension( $orderitem_measurement_data['width']['value'], $wc_dimension_unit, $orderitem_measurement_data['width']['unit'] ) ;
							}
							if( ! empty($orderitem_measurement_data['height']['value']) ) {
								$package['contents'][$key]['height'] = wc_get_dimension($orderitem_measurement_data['height']['value'], $wc_dimension_unit, $orderitem_measurement_data['height']['unit'] ) ;
							}
						}
					}
				}
