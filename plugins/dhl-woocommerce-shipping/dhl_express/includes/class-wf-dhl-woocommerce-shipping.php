...
class wf_dhl_woocommerce_shipping_method extends WC_Shipping_Method {
    ...
    **/
    ...
    private function get_dhl_requests($dhl_packages, $package) {
		...
		$package['dhl_insurance'] = isset($package['dhl_insurance']) ? $package['dhl_insurance'] : true;
        
        if(strpos($this->http_req_referer, 'checkout') > 0){
            if($this->settings['insure_contents'] == 'yes'){
                if(isset($this->dhl_insurance_at_checkout)){
                    if($this->dhl_insurance_at_checkout == 'yes'){
		...
		if((isset($package['dhl_insurance']) && $package['dhl_insurance'] == true) || is_cart())
		{
			...
			$additional_insurance_details = ($this->insure_contents && ($this->conversion_rate || $this->insure_converstion_rate)) ? "<QtdShp><QtdShpExChrg><SpecialServiceType>II</SpecialServiceType><LocalSpecialServiceType>XCH</LocalSpecialServiceType></QtdShpExChrg></QtdShp>" : ""; //insurance type
        } 
        //If vendor country set, then use vendor address
        if(isset($package['origin'])){
            if(isset($package['origin']['country']) && isset($this->settings['vendor_check']) && $this->settings['vendor_check'] === 'yes'){
...
<p:DCTRequest xmlns:p="http://www.dhl.com" xmlns:p1="http://www.dhl.com/datatypes" xmlns:p2="http://www.dhl.com/DCTRequestdatatypes" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.dhl.com DCT-req.xsd ">
  <GetQuote>
    ...
    <From>
	  <CountryCode>{$this->origin_country_1}</CountryCode>
...
XML;
    ...
    public function calculate_shipping( $packages=array() ) {
		...
		foreach($parcels as $parcel){
            ...
            if(strpos($http_referer, 'checkout') > 0){
                if($this->settings['insure_contents'] == 'yes'){
                    ...
                    if(isset($str['dhl_insurance'])){
                        ...
                        $this->dhl_insurance_at_checkout = 'yes';
                    }else{
            ...
            if(strpos($req_url, 'add_to_cart') > 0){
                if($this->settings['insure_contents'] == 'yes'){
                    update_option('dhl_insurance', 'yes');
                    $parcel['dhl_insurance'] = true;
                }else{
    ...
    private function get_result($request) {
        $this->debug('DHL REQUEST: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#3d9cd2;border:1px solid #DDD;padding:5px;">' . print_r(htmlspecialchars($request), true) . '</pre>');
		...
		if(!empty($result))
		{
			$this->debug('DHL RESPONSE: <a href="#" class="debug_reveal">Reveal</a><pre class="debug_info" style="background:#3d9cd2;border:1px solid #DDD;padding:5px;">' . print_r(htmlspecialchars($result), true) . '</pre>');
        }
    ...
    private function prepare_rate($rate_code, $rate_id, $rate_name, $rate_cost, $delivery_time, $rate_compain = '0',$dhl_inurance = false,$shipping_rates_source_currency) {
        // Name adjustment
        ...
        $this->found_rates[$rate_id] = apply_filters('wf_dhl_shipping_found_rate' , array(
            ...
            'sort' => $sort,
            'packages' => $packages,
            'meta_data' => array('dhl_delivery_time'=>$delivery_time,'weight_charge'=>floatval($rate_compain),'extra_charge'=>$extra_charge,'insurance' => (($dhl_inurance) ? 'yes' : 'no'))
