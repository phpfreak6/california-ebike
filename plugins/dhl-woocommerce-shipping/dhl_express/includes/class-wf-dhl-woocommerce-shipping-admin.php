...
class wf_dhl_woocommerce_shipping_admin{
    ...
    public function __construct(){
        ...
        $this->label_contents_text				= isset( $this->settings['label_contents_text'] ) ? $this->settings['label_contents_text'] : 'Shipment Contents';
        ...
        $this->pickup_enable					= isset( $this->settings['add_picup'] ) && $this->settings['add_picup'] =='yes' ? true : false;
        $this->user_settings = get_option('woocommerce_wf_dhl_shipping_settings');
        //$this->customer_insurance = false;
        ...
        if ( isset( $_GET['wf_dhl_process_picup_packages'] ) ) {
            add_action( 'init', array( $this, 'wf_dhl_process_picup_packages' ), 15 );
        }
    ...
    function wf_dhl_delete_label()
    {
        ...
        delete_post_meta($get_id,'_wf_dhl_picup_shipment');
        delete_post_meta($get_id,'_wf_dhl_picup_shipment_error');
        ...
        foreach ($get_stored_packages as $package) {
            if(!empty($package))
            {
                    foreach ($package as $key => $value) {
                        ...
                        $is_insurance = get_option('dhl_insurance_in_order_'.$orderid);
                        if($insurance_arr){
    ...
    function get_service_request($order,$dhl_packages)
    {
        ...
        $is_insurance = get_option('dhl_insurance_in_order_'.$orderid);
        $is_insurance_checkout = get_option('dhl_insurance_in_settings_for_checkout'.$orderid);
        $order_insurance_conversion_rate = get_option('dhl_insurance_conversion_rate_'.$orderid);

        if ($this->settings['insure_contents'] == 'yes' && !empty($this->conversion_rate))
        {
            if($is_insurance_checkout == 'yes'){
                if($is_insurance == 'yes'){
        ...
        if ($this->settings['insure_contents'] == 'yes' && !empty($order_insurance_conversion_rate))
        {   
            if($is_insurance_checkout == 'yes'){
                if($is_insurance == 'yes'){
                    $total_insurance_value = $total_value * $order_insurance_conversion_rate;   
                }else{
		...
		$additional_insurance_details = ($this->insure_contents && ($this->conversion_rate || $order_insurance_conversion_rate)) ? "<QtdShp><QtdShpExChrg><SpecialServiceType>II</SpecialServiceType><LocalSpecialServiceType>XCH</LocalSpecialServiceType></QtdShpExChrg></QtdShp>" : ""; //insurance type
        
        $shipping_country = wf_get_order_shipping_country($order);

        $destination_city = strtoupper(wf_get_order_shipping_city($order));
        $destination_postcode = wf_get_order_shipping_postcode($order);	
...
function wf_dhl_process_picup_packages()
{
	if( !$this->wf_user_permission() ) {
		echo "You don't have admin privileges to view this page.";
		exit;
	...
	$post_id	=	$_GET['wf_dhl_process_picup_packages'];
	...
	$woo_dhl_wrapper->wf_picup_request_handler($order);
...
public function wf_create_shipment($order){		
	...
	$orderid = wf_get_order_id($order);
	$woo_dhl_wrapper->print_label($order,$serviceCode,$orderid);
}
...
public function wf_create_return_shipment($order){		
	...
	$orderid = wf_get_order_id($order);
	$woo_dhl_wrapper->print_return_label($order,$serviceCode,$orderid);
}
...
public function wf_dhl_metabox_content(){
    ...
    $order_insurance = get_option('dhl_insurance_'.$orderid);
    $order_insurance_enabled = get_option('dhl_insurance_in_order_'.$orderid);
    $order_insurance_checkout_enabled = get_option('dhl_insurance_in_settings_for_checkout'.$orderid);
    $order_insurance_conversion_rate = get_option('dhl_insurance_conversion_rate_'.$orderid);
    
    
    $insurance = '';
    $is_insurance = '';
    $is_insurance_checkout = '';
    $current_order_insurance_conversion_rate = 0;
    
    if($order_insurance){
       $insurance = $order_insurance;
    }else{
        $insurance = get_option('dhl_insurance');
        update_option('dhl_insurance_'.$orderid, $insurance);
    }
    
    if($order_insurance_enabled){
       $is_insurance = $order_insurance_enabled;
    }else{
        $is_insurance = get_option('dhl_insurance_at_checkout');
        update_option('dhl_insurance_in_order_'.$orderid, $is_insurance);
    }
    
    if($order_insurance_checkout_enabled){
       $is_insurance_checkout = $order_insurance_checkout_enabled;
    }else{
        $is_insurance_checkout = $this->user_settings['insure_contents_chk'];
        update_option('dhl_insurance_in_settings_for_checkout'.$orderid, $is_insurance_checkout);
    }
    
    if($order_insurance_conversion_rate){
       $current_order_insurance_conversion_rate = $order_insurance_conversion_rate;
    }else{
        $current_order_insurance_conversion_rate = $this->insure_converstion_rate;
        update_option('dhl_insurance_conversion_rate_'.$orderid, $current_order_insurance_conversion_rate);
    }
	...
	$shipping_method_datas = $order->get_shipping_methods();
	...
	if (!empty($shipmentIds)) {
		...
		if(!empty($selected_service) && !empty($this->services[$selected_service]) ){
			...
			$xa_order_to_country = wf_get_order_shipping_country($order);
		...
		foreach($shipmentIds as $shipmentId) {
			...
			$stored_pickup_shipment	=	get_post_meta( $post->ID, '_wf_dhl_picup_shipment', array() );
			$stored_pickup_shipment_error	=	get_post_meta( $post->ID, '_wf_dhl_picup_shipment_error', '' );
			if(!empty($stored_pickup_shipment))
			{
				$conf_num = isset($stored_pickup_shipment[0][0]) ? " <b>Confirmation Number:</b> " . (string) $stored_pickup_shipment[0][0] : '';  
				$pic_date = isset($stored_pickup_shipment[0][1]) ? " <b>Next Possible Pickup Date:</b> " . (string) $stored_pickup_shipment[0][1] : '';  
				echo "<div> <b>Pickup Booked</b> " .$conf_num . $pic_date ."</div>";
			}
			...
			if(!empty($commercial_invoice)){
				...
				if($this->pickup_enable)
				{
					...
					{
					?>
						 <a class="button tips dhl_pickup_generate_packages button-primary" style="text-align: center;float:right;margin-left: 2px;" href="<?php echo admin_url( '/?wf_dhl_process_picup_packages='.$post->ID ); ?>" data-tip="<?php _e( 'Create Pickup Request', 'wf-shipping-dhl' ); ?>"><?php _e( 'Create Pickup Request', 'wf-shipping-dhl' ); ?></a> 
                    ...
                    }else
                    {
                        ...
                        echo '<hr><span style="font-weight:bold;">'.__( 'Return Package(s)' , 'wf-shipping-dhl').': </span> ';
                                        ...
                                        $xa_plt_allowed_country = array('AL','AS','AD','AO','AI','AG','AR','AW','AU','CX','NF','AT','BS','BH','BB','BE','BZ','BJ',
                                                                ...
                                                                'PT','PR','RE','RO','RW','WS','SM','ST','SA','SN','SC','SL','SG','SK','SI','SB','SO','ZA','SS','ES',
                                                                'LK','SD','LC','SR','SJ','SZ','SE','CH','RS','SH','BL','SX','VC','TW','TZ','TH','TG','TK','TO','TT',
                                                                'TR','TC','TV','UG','AE','UK','US','UY','VU','VE','VG','VI','EH');
                        if($this->plt && in_array($xa_order_to_country, $xa_plt_allowed_country))
                        {
		...
		if(empty($stored_packages)	&&	!is_array($stored_packages)){
					...
					foreach($stored_packages as $package_group_key	=>	$package_group){
						if( !empty($package_group) && is_array($package_group) ){ //package group may empty if boxpacking and product have no dimensions 
							foreach($package_group as $stored_package_key	=>	$stored_package){
                                ...
                                if(is_array($dimensions)){   
                                    if($insurance == 'yes'){
                                        if(!empty($dimensions['insurance']) && $dimensions['insurance'] !=0 && !empty($current_order_insurance_conversion_rate))
                                        {
                                            $calc_insurance	=	 $dimensions['insurance'] * $current_order_insurance_conversion_rate;
                                    ...
                                    ?>
                                    <tr>
                                        <!-- <td><select id="dhl_manual_service" name="dhl_manual_service[]" class="dhl_manual_service wc-enhanced-select">
                                        <?php echo $select_box_value; ?></select></td>
                            -->			<td style="width:25%;padding:5px;border-radius:5px;margin-left:4px;"><small><?php echo $product_details; ?></small></td>     
                                        ...
                                        if($insurance == 'yes'){
                                            ?>
                                            <td><input type="text" id="dhl_manual_insurance" name="dhl_manual_insurance[]" style="width:60%;padding:5px;border-radius:5px;margin-left:4px;" value="<?php echo ($customer_insurance) ? $calc_insurance : 0;?>" title="<?php echo $dimensions['insurance'] .' '.$cus_site_currency ?>" /> <b><?php echo (!empty($this->insure_currency)) ? $this->insure_currency : $cus_site_currency; ?> </b> </td>
                                            <?php
                                        }
                                        ?>
				...
				jQuery(document).ready(function(){
					jQuery('#wf_dhl_add_package').on("click", function(){
						var new_row = '<tr>';
                            ...
                            new_row 	+= '<td><input type="text" id="dhl_manual_insurance" name="dhl_manual_insurance[]" style="width:60%;padding:5px;border-radius:5px;margin-left:4px;" value="0"> <b><?php echo (!empty($this->insure_currency)) ? $this->insure_currency : get_woocommerce_currency(); ?></b></td>';
			...
			<li>
				<a class="button tips onclickdisable dhl_create_shipment button-primary" style="" href="<?php echo $generate_url; ?>" data-tip="<?php _e('Create Shipment', 'wf-shipping-dhl'); ?>"><?php _e('Create Shipment', 'wf-shipping-dhl'); ?></a> 
			</li></ul><?php
		} ?>
		<script type="text/javascript">
